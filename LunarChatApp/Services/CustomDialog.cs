...
public sealed class CustomDialogManager
{
    internal event EventHandler<CustomDialogShownEventArgs>? OnDialogShown;
    internal event EventHandler<CustomDialogClosedEventArgs>? OnDialogClosed;
    ...
    internal void Show(Control control, DialogOptions options)
    {
        if (Dialogs.Count > 0)
        {
            ...
            if (existingCustomDialog is null) return;

            var last = Dialogs.Last();
            if (last.Key != control)
            {
                OnDialogClosed?.Invoke(this, new CustomDialogClosedEventArgs { ReplaceExisting = true, Control = last.Key });
            }
        }

        Dialogs.TryAdd(control, options);
        OnDialogShown?.Invoke(this, new CustomDialogShownEventArgs { Control = control, Options = options });
    }
    ...
    internal void CloseDialog(Control control)
    {
        ...
        OnDialogClosed?.Invoke(this, new CustomDialogClosedEventArgs
        {
    ...
    internal void OpenLast()
    {
        ...
        OnDialogShown?.Invoke(this, new CustomDialogShownEventArgs { Control = lastDialog.Key, Options = lastDialog.Value });
    }
    ...
    /// <returns></returns>
    public CustomDialogManager Register<TView, TContext>() where TView : Control
    {
...
internal sealed class CustomDialogShownEventArgs : EventArgs
{
    public Control Control { get; set; } = null!;
    public DialogOptions Options { get; set; } = null!;
}

internal sealed class CustomDialogClosedEventArgs : EventArgs
{
